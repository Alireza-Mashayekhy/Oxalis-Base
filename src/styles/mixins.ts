...
export const narrowScroll = css`
  ...
  &::-webkit-scrollbar-track {
    background: ${({ theme }) => theme.secondary || '#f0f0f0'}; /* Default track color */
  }

  &::-webkit-scrollbar-thumb {
    background: ${({ theme }) => theme.scrollbarThumb || '#888'}; /* Default thumb color */
    border-radius: 5px; /* Optional: adds rounded corners to the scrollbar thumb */
  }

  &::-webkit-scrollbar-thumb:hover {
    background: ${({ theme }) => theme.scrollbarThumbHover || '#555'}; /* Color on hover */
  }
`;
