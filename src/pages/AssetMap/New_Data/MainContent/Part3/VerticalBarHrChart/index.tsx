...
const ApexBarChart = () => {
  ...
  useEffect(() => {
    ...
    try {
      const salaryByDepartment: { [key: string]: { totalSalary: number; jobTitles: Set<string> } } = {};

      data.forEach(({ department, job_title, salary }) => {
        if (!salaryByDepartment[department]) {
          salaryByDepartment[department] = { totalSalary: 0, jobTitles: new Set() };
        }
      ...
      const averageSalaries = Object.entries(salaryByDepartment).map(([department, { totalSalary, jobTitles }]) => ({
        department,
  ...
  const series = [{
    data: departmentSalaries.map(department => Math.floor(department.averageSalary)) // حذف قسمت اعشاری
  }];
  ...
  const options: ApexOptions = {
    ...
    tooltip: {
      custom: ({ series, seriesIndex, dataPointIndex, w }) => {
        const averageSalary = Math.floor(series[seriesIndex][dataPointIndex]);
    ...
    xaxis: {
      categories: departmentSalaries.map(department => department.department),
      labels: {
        style: {
          cssClass: 'xaxis',
          fontFamily: 'IRANSans',
