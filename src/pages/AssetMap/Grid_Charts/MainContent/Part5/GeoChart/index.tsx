...
const MapChart = () => {
    ...
    useEffect(() => {
        ...
        const cityQuantities = dataChart.reduce((acc, item) => {
            if (cityCodes[item.city]) {
                acc[cityCodes[item.city]] =
                    (acc[cityCodes[item.city]] || 0) + item.quantity;
            }
            return acc;
        }, {});

        const formattedData = Object.entries(cityQuantities).map(
    ...
    const options = {
        ...
        title: {
            text: 'نقشه مقادیر تولید در هر شهر',
            style: {
                fontFamily: 'IRANSans',
                fontSize: '14px',
        ...
        series: [
            {
                ...
                dataLabels: {
                    ...
                    formatter () {
                        ...
                        return cityNames[this.point.properties['hc-key']]
                            ? cityNames[this.point.properties['hc-key']]
                            : null;
                    },
                },
            },
        ...
        tooltip: {
            ...
            formatter () {
                ...
                const cityName =
                    cityNames[this.point.properties['hc-key']] || 'نامشخص';
                return `<b>${cityName}</b> <br />
        مقدار تولید: ${this.point.value.toLocaleString()}</b>`;
            },
        },
    };
