...
import { getManFilterData } from '@/selectors/state';
...
const StackedAreaChart: React.FC = () => {
  ...
  const cityKeys = Array.from(new Set((filterData.length > 0 ? filterData : chartData).flatMap(item => Object.keys(item).filter(key => key !== 'jalali_date'))));
  
  const processedData = filterData.length > 0 ? filterData.map(({ jalali_date, quantity, city }) => {
      const [year, month] = jalali_date.split('/');
      const newDate = `${year}/${month}`;
      return { jalali_date: newDate, quantity, city };
    }).reduce((acc, item) => {
      const dateEntry = acc.find(entry => entry.jalali_date === item.jalali_date) || { jalali_date: item.jalali_date };
      dateEntry[item.city] = (dateEntry[item.city] || 0) + item.quantity;
