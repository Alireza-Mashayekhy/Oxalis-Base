...
const Example = () => {
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div style={{ backgroundColor: '#333', padding: '5px', border: '1px solid #ccc', borderRadius: '4px' }}>
  ...
  const CustomLegend = ({ payload }) => {
    return (
      <ul style={{ display: 'inline-flex', listStyleType: 'none', padding: 0, flexDirection: 'column',  border:'1px solid #ccc', borderRadius:'4px', margin:'10px' , 
        background: `${
          theme === "dark" ? lightTheme.textColor : darkTheme.textColor
        }`
      }}>
        {payload.map((entry, index) => (
          <li key={`item-${index}`} style={{ marginRight: '20px', display: 'flex', alignItems: 'center', paddingLeft: '15px', paddingTop: '5px' }}>
            <div style={{ width: '12px', height: '12px', margin: '4px', backgroundColor: PRODUCT_COLORS[entry.name] || '#ccc' }} />
  ...
  return (
    <>
        <h5 style={{textAlign:'center' , color:'#808080' , marginTop: '20px'}}>توزیع درآمد محصول</h5>
        ...
        <PieChart>
          ...
          >
            {data.map((entry, index) => (
              <Cell key={`cell-${index}`} fill={PRODUCT_COLORS[entry.name] || '#ccc'} />
            ))}
          </Pie>
