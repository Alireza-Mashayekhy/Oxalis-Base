...
const ProfitLinChart: React.FC = () => {
  ...
  return (
    <>
      <h5 style={{textAlign:'center' , color:'#808080'}}>معیار های مالی </h5>
      <ResponsiveContainer height={350} width="100%">
        ...
        >
          ...
          <Legend
            formatter={(value) => {
              const index = Lines.findIndex(line => line.key === value);
              return index !== -1 ? Lines[index].name : value;
            }}
            iconType="circle"
            iconSize={10}
            margin={{ left: 10, right: 10 }}
            wrapperStyle={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '5px' }}
          />
          {Lines.map((line, index) => (
            <Line key={line.key} type="monotone" dataKey={line.key} dot={false} stroke={colors[index % colors.length]} />
          ))}
        </LineChart>
      </ResponsiveContainer>
    </>
