...
const MainContent: SFC = () => {
  ...
  const [wageData, setWageData] = useState<any[]>(useSelector(getFeeData)?.data);
  ...
  useEffect(() => {
    window.addEventListener('resize', () => setTableHeight(window.innerHeight - 550));
  }, []);
  ...
  const downloadRow = async (e) => {
    ...
    const params: { [key: string]: string | number | boolean } = {
      ...
      start_date: startDate ? convertToPersianDate(startDate?.toISOString()) : '',
              ...
              <>
                <div className='flex items-center justify-center gap-2 flex-wrap'>
                  <div className='data-filter-inputs flex justify-center items-center gap-6 w-full flex-wrap'>
                    ...
                    <div className='flex  gap-2 flex-col'>
                      <label htmlFor='stockId' className='whitespace-nowrap text-start'>
                        نرخ کارمزد (%) :
                      </label>
                      <S.NumInput
                        value={wage}
