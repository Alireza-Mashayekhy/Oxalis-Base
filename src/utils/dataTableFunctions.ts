...
function createEmptyDataObject(item, nameKey, nameValue) {
  // Create an object with all keys from the item, but with empty values
  const emptyData = Object.keys(item).reduce((acc, key) => {
    acc[key] = ""; // Set each field to an empty string
    return acc;
  }, {});

  // Set the name and DAY_VALUE separately with actual values
  emptyData[nameKey] = nameValue;
  emptyData.DAY_VALUE = 0;

  return emptyData;
}
export const createDepositeTreeTableDataOldVersion = (
  data: Deposite_Frame[]
): TreeNode[] => {
  const treeData: Map<string, TreeNode> = new Map();
  ...
  data.forEach((item) => {
    const ventureTypeKey = item.VENTURE_TYPE;
    const ventureNameKey = `${ventureTypeKey}_${item.VENTURE_NAME}`;
    const assetTypeKey = `${ventureNameKey}_${item.BANK}`;
...
export const createShareTreeTableDataOldVersion = (
  data: ShareFrame[]
): TreeNode[] => {
  const treeData: Map<string, TreeNode> = new Map();
  ...
  data.forEach((item) => {
    const ventureTypeKey = item.VENTURE_TYPE;
    const ventureNameKey = `${ventureTypeKey}_${item.VENTURE_NAME}`;
    const assetTypeKey = `${ventureNameKey}_${item.INDUSTRY_GROUP}`;
    ...
    if (!assetTypeNode) {
      assetTypeNode = {
        key: assetTypeKey,
        data: { name: item.INDUSTRY_GROUP, DAY_VALUE: 0 },
        children: [],
      };
      ventureNameNode.children.push(assetTypeNode);
    }
    ...
    // Add all other item fields to the ASSET_TYPE node, excluding VENTURE_TYPE, VENTURE_NAME, and ASSET_TYPE
    const { VENTURE_TYPE, VENTURE_NAME, INDUSTRY_GROUP, ...remainingFields } =
      item;
    assetTypeNode.children.push({
      key: `${assetTypeKey}_${item._id}`,
      data: remainingFields,
...
export const createBondTreeTableDataOldVersion = (
  data: BondFrame[]
): TreeNode[] => {
  const treeData: Map<string, TreeNode> = new Map();
  ...
  data.forEach((item) => {
    const ventureTypeKey = item.VENTURE_TYPE;
    const ventureNameKey = `${ventureTypeKey}_${item.VENTURE_NAME}`;
    const assetTypeKey = `${ventureNameKey}_${item.ASSET}`;
    ...
    // Add all other item fields to the ASSET_TYPE node, excluding VENTURE_TYPE, VENTURE_NAME, and ASSET_TYPE
    const { VENTURE_TYPE, VENTURE_NAME, SYMBOL, ...remainingFields } = item;
    assetTypeNode.children.push({
      key: `${assetTypeKey}_${item._id}`,
      data: remainingFields,
    });
    ...
    assetTypeNode.children.forEach((assetNameNode) => {
      if (assetNameNode.data.TOTAL_WEIGHT > 0) {
        assetNameNode.data.YTM_MOMENT =
          assetNameNode.data.YTM_MOMENT / assetNameNode.data.TOTAL_WEIGHT;
      }
...
export const CFValueBodyTemplate = (data: CashFlowFrame[]) => {
  return data.data.CASH_FLOW.toLocaleString("en-US");
};
