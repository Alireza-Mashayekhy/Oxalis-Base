...
import { BondFrame, Data, Deposite_Frame, ShareFrame } from '@/types';
...
export const createTreeTableDataOldVersion = (data: Data[]): TreeNode[] => {
  ...
  data.forEach((item) => {
    ...
    let ventureNameNode = ventureTypeNode.children.find((node) => node.key === ventureNameKey);
    ...
    let assetTypeNode = ventureNameNode.children.find((node) => node.key === assetTypeKey);
    if (!assetTypeNode) {
      assetTypeNode = {
        key: assetTypeKey,
    ...
    const { VENTURE_TYPE, VENTURE_NAME, ASSET_TYPE, ...remainingFields } = item;
...
export const createTreeTableDataWithWeight = (data: Data[]): TreeNode[] => {
  ...
  treeData.forEach((ventureTypeNode) => {
    if (ventureTypeNode.data.TOTAL_WEIGHT > 0) {
      ventureTypeNode.data.EFFECTIVE_YIELD /= ventureTypeNode.data.TOTAL_WEIGHT;
    }
    ventureTypeNode.children.forEach((ventureNameNode) => {
      if (ventureNameNode.data.TOTAL_WEIGHT > 0) {
        ventureNameNode.data.EFFECTIVE_YIELD /= ventureNameNode.data.TOTAL_WEIGHT;
      }
      ventureNameNode.children.forEach((assetTypeNode) => {
        if (assetTypeNode.data.TOTAL_WEIGHT > 0) {
          assetTypeNode.data.EFFECTIVE_YIELD /= assetTypeNode.data.TOTAL_WEIGHT;
        }
      });
    });
  });
...
export const createCashFlowTreeTableData = (data) => {
  ...
  data.forEach((item) => {
    ...
    let symbolNode = dateNode.children.find((node) => node.key === symbolKey);
    if (!symbolNode) {
      symbolNode = {
        key: symbolKey,
    ...
    let ventureTypeNode = symbolNode.children.find((node) => node.key === ventureTypeKey);
    if (!ventureTypeNode) {
      ventureTypeNode = {
        key: ventureTypeKey,
        data: { name: item.VENTURE_TYPE, CASH_FLOW: 0 },
    ...
    const { DATE, SYMBOL, VENTURE_TYPE, VENTURE_NAME, ...remainingFields } = item;
...
export const createDepositeTreeTableData = (data: Deposite_Frame[]): TreeNode[] => {
  const treeData: Map<string, TreeNode> = new Map();
  ...
  data.forEach((item) => {
    ...
    // Retrieve or create the VENTURE_NAME node within the VENTURE_TYPE node
    let ventureTypeNode = bankNode.children.find((node) => node.key === ventureTypeKey);
    if (!ventureTypeNode) {
      ventureTypeNode = {
        key: ventureTypeKey,
        data: { name: item.VENTURE_TYPE, DAY_VALUE: 0 },
...
export const createDepositeTreeTableDataWithExpansion = (data) => {
  ...
  data.forEach((item) => {
    ...
    let accountTypeNode = bankNode.children.find((node) => node.key === accountTypeKey);
    if (!accountTypeNode) {
      accountTypeNode = {
        key: accountTypeKey,
        data: {
    ...
    let ventureTypeNode = accountTypeNode.children.find((node) => node.key === ventureTypeKey);
    if (!ventureTypeNode) {
      ventureTypeNode = {
        key: ventureTypeKey,
        data: {
    ...
    const { BANK, ACCOUNT_TYPE, VENTURE_TYPE, VENTURE_NAME, ...remainingFields } = item;
    ventureNameNode.children.push({
      key: `${ventureNameKey}_${item.id}`,
      data: remainingFields
    });
  ...
  treeData.forEach((bankNode) => {
    if (bankNode.data.TOTAL_WEIGHT > 0) {
      bankNode.data.REAL_PROFIT = bankNode.data.REAL_PROFIT / bankNode.data.TOTAL_WEIGHT;
    }
    bankNode.children.forEach((accountTypeNode) => {
      if (accountTypeNode.data.TOTAL_WEIGHT > 0) {
        accountTypeNode.data.REAL_PROFIT =
          accountTypeNode.data.REAL_PROFIT / accountTypeNode.data.TOTAL_WEIGHT;
      }
      accountTypeNode.children.forEach((ventureTypeNode) => {
        ...
        ventureTypeNode.children.forEach((ventureNameNode) => {
          if (ventureNameNode.data.TOTAL_WEIGHT > 0) {
            ventureNameNode.data.REAL_PROFIT =
              ventureNameNode.data.REAL_PROFIT / ventureNameNode.data.TOTAL_WEIGHT;
          }
        });
      });
    });
...
export const createShareTreeTableData = (data: ShareFrame[]): TreeNode[] => {
  ...
  data.forEach((item) => {
    ...
    // Retrieve or create the ASSET_TYPE node within the VENTURE_NAME node
    let vantureNameNode = ventureTypeNode.children.find((node) => node.key === ventureNameKey);
    if (!vantureNameNode) {
      vantureNameNode = {
        key: ventureNameKey,
        data: {
...
export const createBondTreeTableData = (data: BondFrame[]): TreeNode[] => {
  ...
  data.forEach((item) => {
    ...
    // Retrieve or create the VENTURE_NAME node within the VENTURE_TYPE node
    let assetNameNode = assetTypeNode.children.find((node) => node.key === assetNameKey);
    if (!assetNameNode) {
      assetNameNode = {
        key: assetNameKey,
    ...
    // Retrieve or create the ASSET_TYPE node within the VENTURE_NAME node
    let ventureNameNode = assetNameNode.children.find((node) => node.key === ventureNameKey);
    if (!ventureNameNode) {
      ventureNameNode = {
        key: ventureNameKey,
        data: { name: item.VENTURE_NAME, DAY_VALUE: 0 },
...
export const createBondTreeTableDataWithWeightedAverage = (data: BondFrame[]): TreeNode[] => {
  const treeData: Map<string, TreeNode> = new Map();
  ...
  treeData.forEach((assetTypeNode) => {
    if (assetTypeNode.data.TOTAL_WEIGHT > 0) {
      assetTypeNode.data.YTM_MOMENT = assetTypeNode.data.YTM_MOMENT / assetTypeNode.data.TOTAL_WEIGHT;
    }
    assetTypeNode.children.forEach((assetNameNode) => {
      if (assetNameNode.data.TOTAL_WEIGHT > 0) {
        assetNameNode.data.YTM_MOMENT = assetNameNode.data.YTM_MOMENT / assetNameNode.data.TOTAL_WEIGHT;
      }
      assetNameNode.children.forEach((ventureNameNode) => {
        if (ventureNameNode.data.TOTAL_WEIGHT > 0) {
          ventureNameNode.data.YTM_MOMENT =
            ventureNameNode.data.YTM_MOMENT / ventureNameNode.data.TOTAL_WEIGHT;
        }
      });
    });
  });
...
export const roundedNumberBodyTemplateForTreeTable = (data, col: ColumnBodyOptions) => {
...
export const numberBodyTemplateForTreeTable = (data, col: ColumnBodyOptions) => {
  const field = data.data[col.field];
...
export const numberBodyTemplateForTreeTableWithBillionFormat = (data, col: ColumnBodyOptions) => {
  const field = data.data[col.field];
  if (field === null || field === undefined || field === '') {
    return '';
  }
...
export const numberBodyTemplateForTreeTableWithMillionFormat = (data, col: ColumnBodyOptions) => {
  const field = data.data[col.field];
  if (field === null || field === undefined || field === '') {
    return '';
  }
...
export const updatedDataArray = (data: bankAccountData[] | EquityDataForDataTable[]) => {
  console.log(data);
...
export const createBondsDataForVentureTypeFilter = (data: BondFrame[]): Option[] => {
...
export const createDataForVentureNameFilter = (data: Data[], ventureType: string): Option[] => {
  const filteredDataBasedOnVentureType = data.filter((item) => item.VENTURE_TYPE === ventureType);

  const filteredData = filteredDataBasedOnVentureType.reduce((accumulator, current) => {
...
export const createDataForAssetTypeFilter = (
  ...
  const filteredDataBasedOnVentureTypeAndVentureName = data.filter(
    (item) => item.VENTURE_TYPE === ventureType && item.VENTURE_NAME === ventureName
  );

  const filteredData = filteredDataBasedOnVentureTypeAndVentureName.reduce((accumulator, current) => {
